...
def register(app, socketio=None):
    ...
    def categories_admin():
        ...
        return render_template('admin/categories.j2.html', 
    ...
    def category_add():
        ...
        try:
            ...
            import re
            if folder_name and not re.fullmatch(r'[A-Za-z0-9_-]+', folder_name):
                if _wants_json_response():
                    return jsonify({'error': 'Имя папки может содержать только латинские буквы, цифры, дефис и подчёркивание'}), 400
                flash('Имя папки может содержать только латинские буквы, цифры, дефис и подчёркивание', 'error')
                return redirect(url_for('categories_admin'))
            ...
            if not display_name or not folder_name:
                if _wants_json_response():
                    return jsonify({'error': 'Название и имя папки не могут быть пустыми'}), 400
            ...
            if app._sql.category_exists([folder_name]):
                if _wants_json_response():
                    return jsonify({'error': 'Категория с таким именем папки уже существует'}), 409
    ...
    def category_edit(category_id):
        ...
        try:
            ...
            if app._sql.category_name_exists_except_ci([display_name, category_id]):
                if _wants_json_response():
                    return jsonify({'error': 'Категория с таким названием уже существует'}), 409
                flash(f'Категория с именем "{display_name}" уже существует', 'error')
                return redirect(url_for('categories_admin'))
            ...
            if enabled == 0:
                ...
                if en_cnt > 0:
                    if _wants_json_response():
                        return jsonify({'error': 'Нельзя выключить категорию: есть включенные подкатегории.'}), 409
    ...
    def category_delete(category_id):
        ...
        try:
            ...
            if sub_cnt > 0:
                if _wants_json_response():
                    return jsonify({'error': 'Нельзя удалить категорию: есть подкатегории'}), 409
                flash('Нельзя удалить категорию: в ней есть подкатегории. Сначала удалите или перенесите подкатегории.', 'error')
                return redirect(url_for('categories_admin'))
    ...
    def subcategory_add():
        ...
        try:
            ...
            if app._sql.subcategory_exists([category_id, folder_name]):
                if _wants_json_response():
                    return jsonify({'error': 'Подкатегория с таким именем папки уже существует'}), 409
                flash(f'Подкатегория с именем папки "{folder_name}" уже существует в этой категории', 'error')
                return redirect(url_for('categories_admin'))
            ...
            for action in ['view', 'edit', 'delete']:
                for scope in ['own', 'group', 'all']:
                    for entity in ['user', 'group']:
                        ...
                        permissions.append(1 if request.form.get(field_name) else 0)
    ...
    def subcategory_delete(subcategory_id):
        ...
        try:
            ...
            if files_cnt > 0:
                if _wants_json_response():
                    return jsonify({'error': 'Нельзя удалить подкатегорию: в ней есть файлы'}), 409
                flash('Нельзя удалить подкатегорию: в ней есть файлы. Сначала удалите или перенесите файлы.', 'error')
                return redirect(url_for('categories_admin'))
    ...
    def api_groups():
        ...
        total_row = app._sql.execute_scalar(f"SELECT COUNT(1) FROM {prefix}_group {where};", params)
        ...
        return jsonify({
            'items': [{'id': d[0], 'name': d[1], 'description': d[2]} for d in data],
            'page': page,
            'page_size': page_size,
            'total': total
        })
    ...
    def api_users():
        ...
        if query:
            ...
            like = f"%{query}%"
            params.extend([like, like])
        total_row = app._sql.execute_scalar(f"SELECT COUNT(1) FROM {prefix}_user {where};", params)
        total = int(total_row[0]) if total_row else 0
        rows = app._sql.execute_query(
            f"SELECT id, login, name, enabled FROM {prefix}_user {where} ORDER BY name LIMIT %s OFFSET %s;",
            params + [page_size, offset]
        )
        return jsonify({
            'items': [{'id': r[0], 'login': r[1], 'name': r[2], 'enabled': r[3]} for r in rows],
            'page': page,
            'page_size': page_size,
            'total': total
        })
    ...
    def api_subcategory_permissions(subcategory_id):
        ...
        try:
            ...
            _log.warning(f"Falling back to basic subcategory fetch for id={subcategory_id}: {e}")
        ...
        try:
            ...
            user_perms = {
                'view_own': 0, 'view_group': 0, 'view_all': 0,
                'edit_own': 0, 'edit_group': 0, 'edit_all': 0,
                'delete_own': 0, 'delete_group': 0, 'delete_all': 0,
            }
            group_perms = {
                'view_own': 0, 'view_group': 0, 'view_all': 0,
                'edit_own': 0, 'edit_group': 0, 'edit_all': 0,
    ...
    @app.route('/api/subcategory/<int:subcategory_id>/permissions', methods=['POST'])
    @login_required
    @require_permissions(SUBCATEGORIES_MANAGE)
    def api_update_subcategory_permissions(subcategory_id):
        ...
        try:
            ...
            args = [
                subcategory.category_id, subcategory.display_name, subcategory.folder_name,
                subcategory.display_order, subcategory.enabled
