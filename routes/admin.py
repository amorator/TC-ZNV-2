...
def register(app, socketio=None):
	...
	def admin_force_refresh_all():
		...
		try:
			...
			if socketio:
				try:
					# Broadcast to all connected clients first (namespace-wide)
					try:
						socketio.emit('force-refresh', payload)
						print("Broadcast force-refresh to all clients")
					except Exception as e:
						print(f"Failed to broadcast force-refresh globally: {e}")
