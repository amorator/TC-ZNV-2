...
class TestUserManager:
    ...
    def __init__(self, base_url: str = None, admin_login: str = None, admin_password: str = None):
        self.base_url = base_url or os.getenv('BASE_URL', 'http://localhost:5000')
        self.admin_login = admin_login or os.getenv('LOGIN', 'admin')
        self.admin_password = admin_password or os.getenv('PASSWORD', 'admin')
        self.session = requests.Session()
        # Если передан CERT_FILE, requests сам подхватит через REQUESTS_CA_BUNDLE/SSL_CERT_FILE
        # Не отключаем verify, чтобы работала строгая проверка TLS
        self.created_users = []
    ...
    def _ensure_server_reachable(self):
        ...
        try:
            response = self.session.head(self.base_url, timeout=5, allow_redirects=True)
        except requests.exceptions.ConnectionError as e:
            raise ConnectionError(f'Server not reachable: {self.base_url}') from e
    ...
    def create_group(self, group_name: str, description: str = None) -> bool:
        ...
        try:
            ...
            response = self.session.post(f'{self.base_url}/groups/add', data=group_data)
    ...
    def delete_user(self, username: str) -> bool:
        ...
        try:
            response = self.session.post(f'{self.base_url}/users/delete', data={'login': username})
            success = response.status_code in [200, 302]
