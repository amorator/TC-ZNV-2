...
def open_categories(d):
    d.get(f'{BASE}/admin/categories')
    time.sleep(0.3)


def context_click(d, element):
    # JS-эмуляция контекстного клика
    d.execute_script("var ev=new MouseEvent('contextmenu', {bubbles:true}); arguments[0].dispatchEvent(ev);", element)
    time.sleep(0.1)
...
def create_category(d, name: str):
    ...
    add_selectors = ["[data-action='add-category']", "#add-category-button", "button.btn-primary", "a[href*='category'] .btn-primary"]
    opener = None
    for sel in add_selectors:
        ...
        if els:
            ...
            break
    if not opener:
        pytest.skip('Add category control not found')
    ...
    while time.time() < end:
        els = d.find_elements('css selector', "[data-testid='categories-modal-add'].show, #addCategoryModal.show, .modal.show")
        if els:
            modal = els[0]
            break
        time.sleep(0.05)
    if not modal:
        pytest.skip('Category modal not shown')
    ...
    for sel in selectors:
        ...
        if els:
            ...
            break
    if not target_input:
        pytest.skip('Category name input not found')
    # Ввод значения (с учётом неинтерактивности — fallback через JS)
    try:
        d.execute_script("arguments[0].scrollIntoView({block:'center'});", target_input)
        target_input.clear(); target_input.send_keys(name)
    except Exception:
        d.execute_script("arguments[0].value=arguments[1]; arguments[0].dispatchEvent(new Event('input',{bubbles:true}));", target_input, name)
    # Сохранение
    save = d.find_elements('css selector', "[data-testid='categories-modal-add'].show .modal-footer .btn-primary, #addCategoryModal.show .modal-footer .btn-primary, .modal.show .modal-footer .btn-primary, .modal.show button[type='submit'], .modal.show .btn-primary")
    if not save:
        pytest.skip('Category save button not found')
    ...
    try:
        ...
        if modals and modals[0].is_displayed():
            ...
            if close_btns:
                close_btns[0].click(); time.sleep(0.2)
            else:
                d.execute_script("document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape'}));"); time.sleep(0.2)
...
def delete_category_via_menu(d, cat_el):
    ...
    menu = None
    for sel in ["[data-testid='categories-context-menu']", '#categoryContextMenu', '.context-menu', '.dropdown-menu.show']:
        els = d.find_elements('css selector', sel)
    ...
    items = d.find_elements('css selector', "[data-action='delete-category'], .dropdown-item, [role='menuitem'], li, a, button")
    target = None
    for it in items:
        ...
        if 'удал' in txt or 'delete' in txt:
            target = it; break
    if not target:
        pytest.skip('Delete item not found in category menu')
    try:
        target.click()
    except Exception:
    ...
    confirm = d.find_elements('css selector', ".modal.show .btn-danger, .modal.show .btn-primary")
    if confirm:
        try:
            confirm[0].click()
        except Exception:
...
def test_category_delete_and_toggle_menu():
    ...
    try:
        ...
        cat_buttons = d.find_elements('css selector', ".category-tab, .categories .nav .nav-link, [data-entity='category']")
        if not cat_buttons:
            ...
            while time.time() < end:
                ...
                if cat_buttons:
                    break
                d.refresh(); time.sleep(0.4)
            if not cat_buttons:
                # Если так и нет — проверяем, что показано пустое состояние с кнопкой добавить
                empty_texts = d.find_elements('css selector', '.empty-state, .alert-info, .placeholder')
                empty_ok = any(e.is_displayed() for e in empty_texts) or ("нет категорий" in (d.page_source.lower()))
                empty_buttons = d.find_elements('css selector', "[data-action='add-category'], #add-category-button, .empty-state .btn")
                assert empty_ok and empty_buttons, 'Neither category created nor empty state shown'
                pytest.skip('Category creation did not reflect yet; empty state present')
        ...
        assert ('отключ' in items_text or 'disable' in items_text or 'toggle' in items_text)
        # Проверяем удаление категории и пустое состояние (кнопки/сообщения)
        delete_category_via_menu(d, target)
        # Пустое состояние: ищем фразы и кнопки действия
        empty_texts = d.find_elements('css selector', "[data-testid='categories-tab'] .empty-state, .empty-state, .alert-info, .placeholder")
        empty_ok = any(e.is_displayed() for e in empty_texts) or ("нет категорий" in (d.page_source.lower()))
        assert empty_ok, 'Empty state not shown after delete'
        empty_buttons = d.find_elements('css selector', "[data-action='add-category'], #add-category-button, .empty-state .btn")
        assert empty_buttons, 'No action buttons in empty state'
    finally:
        d.quit()
...
def test_subcategory_delete_protection_smoke():
    ...
    try:
        ...
        sub_tabs = d.find_elements('css selector', "#subcategory-nav .topbtn, .subcategory-tab, .subcategories .nav .nav-link, [data-entity='subcategory']")
        if not sub_tabs:
            pytest.skip('No subcategories found to test')
        sub = sub_tabs[0]
        context_click(d, sub)
        menu = None
        for sel in ["[data-testid='categories-context-menu']", '#subcategoryContextMenu', '.context-menu', '.dropdown-menu.show']:
            els = d.find_elements('css selector', sel)
            if els and els[0].is_displayed():
                menu = els[0]
                break
...
def test_categories_context_menu_items():
    ...
    try:
        ...
        if not cat_buttons:
            ...
            cat_buttons = d.find_elements('css selector', ".category-tab, .categories .nav .nav-link, [data-entity='category']")
            if not cat_buttons:
                pytest.skip('Category creation failed or not reflected')
        ...
        menu_items = menu.find_elements('css selector', 'li, a, button, [role="menuitem"]')
        menu_texts = [item.text.lower().strip() for item in menu_items if item.text]
        
        expected_items = ['удал', 'delete', 'отключ', 'disable', 'toggle', 'редакт', 'edit']
        found_items = [item for item in expected_items if any(item in text for text in menu_texts)]
        
        assert len(found_items) >= 2, f'Expected at least 2 menu items, found: {found_items}'
        ...
        if sub_tabs:
            ...
            sub_menu_items = sub_menu.find_elements('css selector', 'li, a, button, [role="menuitem"]')
            sub_menu_texts = [item.text.lower().strip() for item in sub_menu_items if item.text]
            
            sub_found_items = [item for item in expected_items if any(item in text for text in sub_menu_texts)]
            assert len(sub_found_items) >= 1, f'Expected at least 1 subcategory menu item, found: {sub_found_items}'
...
def test_categories_delete_confirmation():
    ...
    try:
        ...
        if not target_cat:
            pytest.skip('Created category not found in UI')
        ...
        delete_item = None
        for item in menu.find_elements('css selector', 'li, a, button, [role="menuitem"]'):
            if item.text and ('удал' in item.text.lower() or 'delete' in item.text.lower()):
                delete_item = item
                break
        ...
        if confirm_modal:
            ...
            confirm_btn = confirm_modal.find_elements('css selector', '.btn-danger, .btn-primary, button[type="submit"]')
            if confirm_btn:
                try:
                    confirm_btn[0].click()
                except Exception:
        ...
        remaining_cats = d.find_elements('css selector', ".category-tab, .categories .nav .nav-link, [data-entity='category']")
        cat_texts = [cat.text.lower() for cat in remaining_cats if cat.text]
        
        assert test_name.lower() not in cat_texts, f'Category {test_name} still present after deletion'
...
def test_categories_empty_state_actions():
    ...
    try:
            ...
            try:
                ...
                if menu:
                    ...
                    if delete_item:
                        ...
                        confirm_modal = d.find_elements('css selector', ".modal.show")
                        if confirm_modal:
                            confirm_btn = confirm_modal[0].find_elements('css selector', '.btn-danger, .btn-primary')
                            if confirm_btn:
                                try:
                                    confirm_btn[0].click()
                                except Exception:
        ...
        empty_indicators = d.find_elements('css selector', '.empty-state, .alert-info, .placeholder, [data-testid="categories-empty"]')
        empty_text_present = "нет категорий" in d.page_source.lower() or "no categories" in d.page_source.lower()
        
        assert empty_indicators or empty_text_present, 'Empty state not shown'
        
        # Проверяем наличие кнопки добавления
        add_buttons = d.find_elements('css selector', "[data-action='add-category'], #add-category-button, .empty-state .btn, .btn-primary")
        ...
        # Проверяем появление модалки добавления
        modal = d.find_elements('css selector', "[data-testid='categories-modal-add'].show, #addCategoryModal.show, .modal.show")
        assert modal, 'Add category modal not shown'
