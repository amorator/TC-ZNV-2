...
def _ensure_target_or_skip():
    ...
    try:
        # Мягкая проверка доступности: GET допускает self-signed
        requests.get(BASE,
                     timeout=8,
                     allow_redirects=True,
                     verify=not ACCEPT_INSECURE_CERTS)
    except Exception:
        # Не скипаем — дадим тесту попытаться и упасть предметно
        return
...
def _open_modal_by_selector(d, page_url: str, open_selectors,
                            modal_visible_selectors):
    d.get(page_url)
    time.sleep(0.3)
    opener = None
    for sel in open_selectors:
        ...
        if els:
            ...
            break
    if not opener:
        pytest.xfail(f'Opener not found on {page_url}')
    try:
        opener.click()
    except Exception:
    ...
    for sel in modal_visible_selectors:
        ...
        if els and els[0].is_displayed():
            return els[0]
    pytest.xfail('Modal not visible')
...
def test_users_add_modal_fields():
    ...
    try:
        login_admin(d)
        modal = _open_modal_by_selector(d, f'{BASE}/users', [
            "[data-testid='users-cm-add']", "[data-testid='users-modal-add']",
            "#popup-add", "button.btn-primary"
        ], [
            "[data-testid='users-modal-add'] .popup", "#popup-add .popup",
            "#popup-add .modal.show", ".modal.show"
        ])
        # Поля логина/имени/пароля/группы
        for sel in [
                '#add-login', '#add-name', '#add-password', '#add-password2',
                '#add-group'
        ]:
            assert d.find_elements('css selector', sel), f'Missing field {sel}'
        # Кнопки
        assert d.find_elements(
            'css selector',
            "[data-testid='users-add-submit']"), 'Missing submit button'
        # Закрыть модалку
        els = d.find_elements(
            'css selector',
            "[data-testid='users-add-cancel'], .modal.show [data-bs-dismiss='modal']"
        )
        if els:
            try:
                els[0].click()
...
def test_groups_add_modal_fields():
    ...
    try:
        login_admin(d)
        modal = _open_modal_by_selector(d, f'{BASE}/groups', [
            "[data-testid='groups-cm-add']",
            "[data-testid='groups-modal-add']", "#popup-add",
            "button.btn-primary"
        ], [
            "[data-testid='groups-modal-add'] .popup", "#popup-add .popup",
            "#popup-add .modal.show", ".modal.show"
        ])
        ...
        assert d.find_elements(
            'css selector',
            "[data-testid='groups-add-submit']"), 'Missing submit button'
        # Закрыть модалку
        els = d.find_elements(
            'css selector',
            "[data-testid='groups-add-cancel'], .modal.show [data-bs-dismiss='modal']"
        )
        if els:
            try:
                els[0].click()
...
def test_files_add_modal_fields():
    ...
    try:
        login_admin(d)
        modal = _open_modal_by_selector(d, f"{BASE}/files", [
            "[data-testid='files-cm-add']", "[data-testid='files-modal-add']",
            "#popup-add", "button.btn-primary"
        ], [
            "[data-testid='files-modal-add'] .popup", "#popup-add .popup",
            "#popup-add .modal.show", ".modal.show"
        ])
        # Поля и кнопки
        ...
        assert d.find_elements(
            'css selector',
            "[data-testid='files-add-submit']"), 'Missing submit button'
        # Закрыть модалку
        els = d.find_elements(
            'css selector',
            "[data-testid='files-add-cancel'], .modal.show [data-bs-dismiss='modal']"
        )
        if els:
            try:
                els[0].click()
...
def test_categories_add_modal_fields():
    ...
    try:
        login_admin(d)
    modal = _open_modal_by_selector(d, f"{BASE}/categories", [
            "[data-action='add-category']",
            "[data-testid='categories-modal-add']", "#addCategoryModal"
        ], [
            "[data-testid='categories-modal-add'].show",
            "#addCategoryModal.show", ".modal.show"
        ])
        for sel in [
                '#add_display_name', '#add_folder_name', '#add_display_order'
        ]:
            assert d.find_elements('css selector', sel), f'Missing field {sel}'
        # Закрытие
        els = d.find_elements(
            'css selector',
            "[data-testid='categories-modal-add'] [data-bs-dismiss='modal'], #addCategoryModal [data-bs-dismiss='modal'], .modal.show .btn-secondary"
        )
        if els:
            try:
                els[0].click()
...
def test_users_add_modal_required_flags():
    ...
    try:
        login_admin(d)
        _open_modal_by_selector(d, f'{BASE}/users', [
            "[data-testid='users-cm-add']", "[data-testid='users-modal-add']",
            "#popup-add", "button.btn-primary"
        ], [
            "[data-testid='users-modal-add'] .popup", "#popup-add .popup",
...
def test_categories_add_modal_required_flags():
    ...
    try:
        login_admin(d)
        _open_modal_by_selector(d, f'{BASE}/categories', [
            "[data-action='add-category']",
            "[data-testid='categories-modal-add']", "#addCategoryModal"
        ], [
            "[data-testid='categories-modal-add'].show",
...
def _is_disabled(d, sel: str) -> bool:
    ...
    try:
        ...
        return (bool(dis)
                and dis != 'false') or (aria == 'true') or ('disabled' in cls)
    except Exception:
        return False
...
def test_groups_add_modal_negative_validation():
    ...
    try:
        login_admin(d)
        _open_modal_by_selector(d, f'{BASE}/groups', [
            "[data-testid='groups-cm-add']",
            "[data-testid='groups-modal-add']", "#popup-add",
            "button.btn-primary"
        ], [
