...
def send_admin_broadcast(d_admin, text: str):
    ...
    openers = d_admin.find_elements('css selector',
                                    "[data-testid='admin-open-notify']")
    if not openers:
    ...
    inputs = d_admin.find_elements(
        'css selector',
        "#notifyTextM, #notify-text, textarea[name='message'], .modal textarea"
    )
    if not inputs:
        pytest.skip('Admin message textarea not found')
    inputs[0].clear()
    inputs[0].send_keys(text)
    send_btns = d_admin.find_elements(
        'css selector',
        "[data-testid='admin-send-notify'], .modal-footer .btn-primary, button[type='submit'], #notify-send"
    )
    if not send_btns:
...
def create_test_user(d_admin, username: str):
    ...
    d_admin.get(f'{BASE}/users')
    time.sleep(0.3)

    # Открываем модалку добавления
    add_btns = d_admin.find_elements(
        'css selector',
        "[data-testid='users-add-open'], [data-testid='users-cm-add'], [data-testid='users-modal-add'], #popup-add button"
    )
    if not add_btns:
    ...
    # Заполняем форму
    login_field = d_admin.find_elements('css selector',
                                        "#add-login, input[name='login']")
    password_field = d_admin.find_elements(
        'css selector', "#add-password, input[name='password']")
    ...
    # Сохраняем
    save_btn = d_admin.find_elements(
        'css selector',
        "[data-testid='users-modal-add'] button[type='submit'], .modal.show button[type='submit']"
    )
    if save_btn:
        try:
            save_btn[0].click()
...
def wait_for_user_in_list(d_user, username: str, timeout: float = 5.0) -> bool:
    ...
    while time.time() < end:
        d_user.refresh()
        time.sleep(0.3)
        users = d_user.find_elements(
            'css selector',
            "[data-testid='users-table'] tbody tr, .users-table tbody tr, table tbody tr"
        )
...
def create_test_file(d_admin, filename: str):
    ...
    d_admin.get(f'{BASE}/files')
    time.sleep(0.3)

    # Открываем модалку добавления файла
    add_btns = d_admin.find_elements(
        'css selector',
        "[data-testid='files-upload'], [data-testid='files-cm-add'], [data-testid='files-modal-add'], #popup-add button"
    )
    if not add_btns:
    ...
    # Заполняем форму
    name_field = d_admin.find_elements('css selector',
    ...
    save_btn = d_admin.find_elements(
        'css selector',
        "[data-testid='files-modal-add'] button[type='submit'], .modal.show button[type='submit']"
    )
    if save_btn:
        try:
            save_btn[0].click()
...
def create_test_group(d_admin, groupname: str):
    ...
    d_admin.get(f'{BASE}/groups')
    time.sleep(0.3)

    # Открываем модалку добавления
    add_btns = d_admin.find_elements(
        'css selector',
        "[data-testid='groups-add-open'], [data-testid='groups-cm-add'], [data-testid='groups-modal-add'], #popup-add button"
    )
    if not add_btns:
    ...
    # Заполняем форму
    name_field = d_admin.find_elements(
        'css selector', "#add-group-name, input[name='group_name']")
    if not name_field:
    ...
    save_btn = d_admin.find_elements(
        'css selector',
        "[data-testid='groups-modal-add'] button[type='submit'], .modal.show button[type='submit']"
    )
    if save_btn:
        try:
            save_btn[0].click()
...
def wait_for_group_in_list(d_user,
                           groupname: str,
    ...
    while time.time() < end:
        d_user.refresh()
        time.sleep(0.3)
        groups = d_user.find_elements(
            'css selector',
            "[data-testid='groups-table'] tbody tr, .groups-table tbody tr, table tbody tr"
        )
...
def wait_user_notification(d_user, text: str, timeout: float = 6.0) -> bool:
    ...
    while time.time() < end:
        ...
        try:
            ...
            toasts = d_user.find_elements(
                'css selector',
                "#toast-container .toast, #toastContainer .toast, .toast, [role='alert']"
            )
            for t in toasts:
...
@pytest.mark.ui
def test_realtime_admin_broadcast_to_user(qa_admin_credentials,
    ...
    try:
        ...
        assert wait_user_notification(
            d_user, marker), 'User did not receive realtime notification'
    finally:
        d_admin.quit()
        d_user.quit()
...
def test_realtime_user_creation_updates(qa_admin_credentials):
    ...
    try:
        ...
        assert wait_for_user_in_list(
            d_admin2,
            test_username), f'User {test_username} not appeared in realtime'
...
@pytest.mark.ui
def test_realtime_file_creation_updates(qa_admin_credentials,
                                        qa_regular_credentials):
    ...
    try:
        ...
        assert wait_for_file_in_list(
            d_user,
            test_filename), f'File {test_filename} not appeared in realtime'
...
def test_realtime_group_creation_updates(qa_admin_credentials):
    ...
    try:
        ...
        assert wait_for_group_in_list(
            d_admin2,
            test_groupname), f'Group {test_groupname} not appeared in realtime'
...
@pytest.mark.ui
def test_realtime_multiple_sessions_notifications(qa_admin_credentials,
                                                  qa_regular_credentials,
...
def test_realtime_cross_role_notifications(all_user_credentials):
    ...
    try:
        ...
        manager_received = wait_user_notification(d_manager,
                                                  marker,
                                                  timeout=6.0)
