...
def _login_session(username: str, password: str) -> requests.Session:
    ...
    s = requests.Session()
    # Разрешаем самоподписанные сертификаты в тестовой среде
    try:
        s.verify = not ACCEPT_INSECURE_CERTS
    except Exception:
        pass
    ...
    if r.url.endswith('/login') or '/login' in r.url:
        pytest.xfail(f'Cannot login as {username}')
    return s
...
def test_permissions_users_admin(qa_admin_credentials):
    ...
    # admin can add user
    r = s.post(f'{BASE}/users/add',
               data={
                   'login': f'perm_u_{int(time.time())}',
                   'password': 'QAtest123!'
               },
...
def test_permissions_users_regular_forbidden(qa_regular_credentials):
    ...
    r = s.post(f'{BASE}/users/add',
               ...
               allow_redirects=False)
    assert _expect_forbidden(
        r.status_code) or r.status_code == 302 and '/login' in (r.headers.get(
            'Location', ''))
...
def test_permissions_groups_manager_can_add(qa_manager_credentials):
    ...
    s = _login_session(user, pwd)
    r = s.post(f'{BASE}/groups/add',
               data={'group_name': f'perm_g_{int(time.time())}'},
               allow_redirects=False)
    assert _expect_success(r.status_code)
...
def test_permissions_files_move_reader_forbidden(qa_reader_credentials):
    ...
    for path in ('/files/move', '/files_move'):
        ...
        if r.status_code != 404:
            assert _expect_forbidden(
                r.status_code) or r.status_code == 302 and '/login' in (
...
def test_rate_limit_users_add_burst(qa_admin_credentials):
    ...
    if not hit_429:
        pytest.skip(
            'Rate limit not triggered (policy may be lenient in this env)')
