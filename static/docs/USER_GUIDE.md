...
### Наряды: загрузка файлов

#### С ПК
Откройте «Наряды», перейдите к нужному наряду и откройте окно файлов (двойным щелчком по строке или через контекстное меню «Файлы»). Нажмите «Загрузить с ПК», выберите один или несколько файлов в системном диалоге и дождитесь завершения загрузки — индикатор прогресса покажет состояние, после чего файлы появятся в списке. При необходимости добавьте или отредактируйте описание файла.
![Загрузка файлов с ПК](images/user/orders_upload_pc.png?v=2025-11-10)

#### С регистратора
В окне файлов нажмите «Загрузить с регистратора» и выберите устройство. Появятся параметры (уровни выбора), заполняйте их по порядку, пока не отобразится список записей. Отметьте нужные записи и запустите импорт — после завершения они появятся в списке файлов наряда. Если регистратор временно недоступен, на первом шаге будет показано понятное сообщение об ошибке, а поля параметров скроются до повторной попытки.
![Загрузка с регистратора](images/user/orders_reg_select.png?v=2025-11-10)
