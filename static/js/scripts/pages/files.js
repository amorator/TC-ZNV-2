  ...
  try {
    ...
    for (const key of uploadKeys) {
      ...
      if (stored) {
        ...
        // Remove old toasts (older than 1 hour)
        if (age > 3600000) {
            ...
            if (data && data.upload) {
              ...
              if (typeof showPersistentProgressIndicator === "function") {
                showPersistentProgressIndicator(
                  ...
                  toastData.totalFiles
                );

                // Update with current progress
                if (typeof updatePersistentProgress === "function") {
                  updatePersistentProgress(
                    toastData.uploadId,
                    toastData.completedFiles,
                    toastData.totalFiles,
                    toastData.currentFile,
                    toastData.currentFileProgress
                  );
                }

                // Resume monitoring (if function is available)
                if (typeof monitorUploadProgress === "function") {
...
function updateMoveSubcategories(selectedCategoryId, subSelect) {
  try {
    ...
    // Prefer data-subs injected into selected option (now contains ID->name mapping)
    let subsMap = null;
    try {
    ...
    if (!subsMap) {
      ...
      // Find the category in dirs data by ID
      let categoryData = null;
      for (let i = 0; i < window.dirsData.length; i++) {
        const categoryObj = window.dirsData[i];
        const keys = Object.keys(categoryObj);
        const categoryKey = keys[0];
        // Try to match by category ID (would need to resolve from folder_name)
        // For now, fall back to name matching
      ...
      // Build subs map from category data (legacy fallback)
      subsMap = {};
    ...
    subKeys.forEach(function (subId) {
      const subName = subsMap[subId];
      const option = document.createElement("option");
      option.value = subId; // Now using ID as value
...
function startUploadWithProgress(form) {
  ...
  function resetAfterUpload() {
    try {
      ...
      // Make cancelUploadRegular globally available
      window.cancelUploadRegular = cancelUploadRegular;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  (function () {
      ...
      try {
        ...
        try {
          var toastData = JSON.parse(
            localStorage.getItem(`upload_toast_${uploadId}`) || "{}"
          );
    ...
    function cancelUpload(uploadId) {
      try {
        // If no uploadId provided, call the regular cancelUpload function
        if (!uploadId) {
          if (typeof window.cancelUploadRegular === "function") {
            window.cancelUploadRegular();
          }
          return;
        }
