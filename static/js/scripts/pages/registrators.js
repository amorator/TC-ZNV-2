// Registrators Page - Restored from working version
// Адаптировано под текущую модульную структуру
...
(function () {
          ...
          fetch("/registrators/" + encodeURIComponent(rid) + "/permissions")
            ...
            .then(function (data) {
              if (data && data.permissions) {
                ...
                // Enforce admin access on fresh data
                regLastSavedPermissions = enforceAdminAccess(
                  regLastSavedPermissions,
                  window.currentGroupsData || [],
                  window.currentUsersData || []
                );
                // Update draft with fresh data
                regCurrentPermissionsDraft = JSON.parse(
                  JSON.stringify(regLastSavedPermissions)
                );
  ...
  function updateUserPermissionsFromGroup(groupId, enabled) {
    try {
      ...
      var usersInGroup = window.currentUsersData.filter(function (user) {
        return String(user.gid || "") === String(groupId);
      });

      if (usersInGroup.length === 0) return;

      // Enable all users in this group
  ...
  // Global function for user permission changes
  window.updateRegistratorUserPermission = function (userId, checked) {
    try {
      ...
      if (isForceUser && !checked) {
        // Re-check the checkbox for force users
        setTimeout(() => {
          var input = document.querySelector(
      ...
      // Update UI to show inheritance indicator
      var userRowElement = document.querySelector(
        `input[data-entity="user"][data-id="${userId}"]`
      );
      if (userRowElement) {
        var userRow = userRowElement.closest("tr");
        var inheritanceText = userRow.querySelector(
          'small.text-muted:contains("(от настроек пользователя)")'
        );
